...
// strip of chips. Each row's first metric is shown inline.
export function StandingsStrip({ rows, title, limit = 10 }: { rows?: StandingRow[]; title?: string; limit?: number }) {
    ...
    <div className="mt-3.5 border border-line rounded-[9px] bg-panel px-3.5 py-3">
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {rows.slice(0, limit).map((d, i) => (
          <div
            key={d.code + i}
            className="flex-none flex items-center gap-2 border border-line2 rounded-md bg-panel2 px-2.5 py-1.5"
          >
            <span
